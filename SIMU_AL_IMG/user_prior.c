///////////////////////////////////
// Auto-generated user_prior.c
// Generated by generate_user_prior.py
//
// Number of parameters: 17
// Prior assignment:
//   para0: trunc_gaussian
//   para1: beta
//   para2: uniform
//   para3: uniform
//   para4: uniform
...
//   para16: uniform
//
// Do not edit repetitive blocks by hand unless necessary.
// Change generate_user_prior.py and regenerate this file instead.
///////////////////////////////////
...
void read_parm_range(char *path)
{
    ...
    //
    ////////////////// para15
    para_name = "para15_max";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%lf", dummy, &para15_max);
    free(para_line);
    para_line = NULL;
    para_name = "para15_min";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%lf", dummy, &para15_min);
    free(para_line);
    para_line = NULL;
    //
    ////////////////// para16
    para_name = "para16_max";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%lf", dummy, &para16_max);
    free(para_line);
    para_line = NULL;
    para_name = "para16_min";
    para_line = read_onepara(path, para_name);
    sscanf(para_line, "%[^:]:%lf", dummy, &para16_min);
    free(para_line);
    para_line = NULL;
    //
}
...
//
double para15_init(double para15_min, double para15_max)
{
    double para15;
    para15 = r8_unif_ab( para15_min, para15_max );
    return para15;
}
//
double para16_init(double para16_min, double para16_max)
{
    double para16;
    para16 = r8_unif_ab( para16_min, para16_max );
    return para16;
}
//
...
void init_parm_set(int seed, double* chain_parm)
{
    ...
    chain_parm[15] = para15_init(para15_min, para15_max);
    chain_parm[16] = para16_init(para16_min, para16_max);
    //
...
int calc_sigma_scale_boundary(double sigma_scale_min, double sigma_scale_max, double *sigma_parm_min, double *sigma_parm_max)
{
    ...
    sigma_parm_min[15] = sigma_scale_min * (para15_max - para15_min);
    sigma_parm_min[16] = sigma_scale_min * (para16_max - para16_min);
    ...
    sigma_parm_max[15] = sigma_scale_max * (para15_max - para15_min);
    sigma_parm_max[16] = sigma_scale_max * (para16_max - para16_min);

    for (int i=0; i<N_parm; i++)
    {
        // each of the following cases is an error
...
{
    *(ptr_sigma_prop) = (para0_max - para0_min) * init_gp_ratio;
    *(ptr_sigma_prop+1) = (para1_max - para1_min) * init_gp_ratio;
    *(ptr_sigma_prop+2) = (para2_max - para2_min) * init_gp_ratio;
    *(ptr_sigma_prop+3) = (para3_max - para3_min) * init_gp_ratio;
    *(ptr_sigma_prop+4) = (para4_max - para4_min) * init_gp_ratio;
    *(ptr_sigma_prop+5) = (para5_max - para5_min) * init_gp_ratio;
    *(ptr_sigma_prop+6) = (para6_max - para6_min) * init_gp_ratio;
    *(ptr_sigma_prop+7) = (para7_max - para7_min) * init_gp_ratio;
    *(ptr_sigma_prop+8) = (para8_max - para8_min) * init_gp_ratio;
    *(ptr_sigma_prop+9) = (para9_max - para9_min) * init_gp_ratio;
    *(ptr_sigma_prop+10) = (para10_max - para10_min) * init_gp_ratio;
    *(ptr_sigma_prop+11) = (para11_max - para11_min) * init_gp_ratio;
    *(ptr_sigma_prop+12) = (para12_max - para12_min) * init_gp_ratio;
    *(ptr_sigma_prop+13) = (para13_max - para13_min) * init_gp_ratio;
    *(ptr_sigma_prop+14) = (para14_max - para14_min) * init_gp_ratio;
    *(ptr_sigma_prop+15) = (para15_max - para15_min) * init_gp_ratio;
    *(ptr_sigma_prop+16) = (para16_max - para16_min) * init_gp_ratio;
    return 0;
}
...
{
    // PingPong outliers inside the allowed prior range
    int debug = 0; // output debug infos
    //
    *ptr_one_chain_new = bounce_inside(*ptr_one_chain_new, para0_min, para0_max);
    *(ptr_one_chain_new+1) = bounce_inside(*(ptr_one_chain_new+1), para1_min, para1_max);
    *(ptr_one_chain_new+2) = bounce_inside(*(ptr_one_chain_new+2), para2_min, para2_max);
    *(ptr_one_chain_new+3) = bounce_inside(*(ptr_one_chain_new+3), para3_min, para3_max);
    *(ptr_one_chain_new+4) = bounce_inside(*(ptr_one_chain_new+4), para4_min, para4_max);
    *(ptr_one_chain_new+5) = bounce_inside(*(ptr_one_chain_new+5), para5_min, para5_max);
    *(ptr_one_chain_new+6) = bounce_inside(*(ptr_one_chain_new+6), para6_min, para6_max);
    *(ptr_one_chain_new+7) = bounce_inside(*(ptr_one_chain_new+7), para7_min, para7_max);
    *(ptr_one_chain_new+8) = bounce_inside(*(ptr_one_chain_new+8), para8_min, para8_max);
    *(ptr_one_chain_new+9) = bounce_inside(*(ptr_one_chain_new+9), para9_min, para9_max);
    *(ptr_one_chain_new+10) = bounce_inside(*(ptr_one_chain_new+10), para10_min, para10_max);
    *(ptr_one_chain_new+11) = bounce_inside(*(ptr_one_chain_new+11), para11_min, para11_max);
    *(ptr_one_chain_new+12) = bounce_inside(*(ptr_one_chain_new+12), para12_min, para12_max);
    *(ptr_one_chain_new+13) = bounce_inside(*(ptr_one_chain_new+13), para13_min, para13_max);
    *(ptr_one_chain_new+14) = bounce_inside(*(ptr_one_chain_new+14), para14_min, para14_max);
    *(ptr_one_chain_new+15) = bounce_inside(*(ptr_one_chain_new+15), para15_min, para15_max);
    *(ptr_one_chain_new+16) = bounce_inside(*(ptr_one_chain_new+16), para16_min, para16_max);
    //
    if (debug)
    {
        ...
        save_debug_para_boundary("para15", *(ptr_one_chain_new+15), para15_min, para15_max);
        save_debug_para_boundary("para16", *(ptr_one_chain_new+16), para16_min, para16_max);
    }
    return 0;
}
...
////////////////////////////////////////////
/*
 * Supported priors generated here:
 *   uniform
 *   beta: fixed Beta(0.867, 3.03), mapped to [para_min, para_max]
 *   trunc_gaussian: mu=(min+max)/2, sigma=(max-min)/10, truncated to [min,max]
 */
//
/////////////////////////
double prior_para0(double para0, double para0_min, double para0_max)
{
    double range = para0_max - para0_min;
    double sigma = range / 10.0;
    double mu = (para0_min + para0_max) / 2.0;

    if (para0 < para0_min || para0 > para0_max) {
        return 0.0;
    }

    double exponent = -0.5 * pow((para0 - mu) / sigma, 2);
    double gaussian_pdf = exp(exponent) / (sigma * sqrt(2 * PI));

    double z_min = (para0_min - mu) / sigma;
    double z_max = (para0_max - mu) / sigma;
...
//
/////////////////////////
double prior_para1(double para1, double para1_min, double para1_max)
{
    ...
    double x = (para1 - para1_min) / (para1_max - para1_min);
    ...
    double jacobian = 1.0 / (para1_max - para1_min);
...
{
    double range = para7_max - para7_min;
    double sigma = range / 10.0;
    double mu = (para7_min + para7_max) / 2.0;

    if (para7 < para7_min || para7 > para7_max) {
        return 0.0;
    }

    double exponent = -0.5 * pow((para7 - mu) / sigma, 2);
    double gaussian_pdf = exp(exponent) / (sigma * sqrt(2 * PI));

    double z_min = (para7_min - mu) / sigma;
    double z_max = (para7_max - mu) / sigma;
    double normalization = 0.5 * (erf(z_max / sqrt(2)) - erf(z_min / sqrt(2)));
...
//
/////////////////////////
double prior_para8(double para8, double para8_min, double para8_max)
{
    // Beta distribution parameters: Beta(0.867, 3.03)
    double alpha = 0.867;
    double beta = 3.03;
    ...
    // Map from [para_min, para_max] to [0, 1]
    double x = (para8 - para8_min) / (para8_max - para8_min);
    ...
    // Change of variables: p(y) = p(x) * |dx/dy| = p(x)/(max-min)
    double jacobian = 1.0 / (para8_max - para8_min);
...
//
/////////////////////////
double prior_para15(double para15_min, double para15_max)
{
    return 1/(para15_max-para15_min);
}
//
/////////////////////////
double prior_para16(double para16_min, double para16_max)
{
    return 1/(para16_max-para16_min);
}
//
...
double log_prior(double *ptr_one_chain)
{
    ...
    para15 = *(ptr_one_chain+15);
    para16 = *(ptr_one_chain+16);

    //
    log_prior = log(prior_para0(para0, para0_min, para0_max)) +
                log(prior_para1(para1, para1_min, para1_max)) +
                ...
                log(prior_para8(para8, para8_min, para8_max)) +
                ...
                log(prior_para13(para13_min, para13_max)) +
                log(prior_para14(para14_min, para14_max)) +
                log(prior_para15(para15_min, para15_max)) +
                log(prior_para16(para16_min, para16_max)) ;
    //
    if (debug)
    {
        printf("para0,para1,para2,para3,para4,para5,para6,para7,para8,para9,para10,para11,para12,para13,para14,para15,para16: %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf\n", para0, para1, para2, para3, para4, para5, para6, para7, para8, para9, para10, para11, para12, para13, para14, para15, para16);
    }
    //
    return log_prior;
}
